...
class LargeInteger{
    ...
    LargeInteger add(LargeInteger B){ // checked
        LargeInteger C = new LargeInteger();
        int carry = 0, i = 0, data, remainder;
        while((i < this.list.size()) && (i < B.list.size())){
            data = this.list.get(i) + B.list.get(i);
            remainder = data % 10;
    ...
    static LargeInteger multiply(LargeInteger A, LargeInteger B, int threshold){
        ...
        if(A.value.equals("0") || B.value.equals("0")){
            ...
            LargeInteger result2 = multiply(X,Z,threshold).add(multiply(W,Y,threshold)).powByExponential(m);
