...
public class DynamicTSP { // 이따가 정리하기.
    static int INF;
    ...
    static void traveling(int[][] graph){
        ...
        TSPNode parentNode = new TSPNode(0);
        parentNode.path.add(1);
        parentNode.bound = bound(parentNode,graph);
        ...
        while(!priorityQueue.isEmpty()){
            ...
            if(parentNode.bound < minLength){
                for(int i = 2; i <= numOfVertex; i++){
                    if(isIn(i,parentNode.path)){
                        continue;
                    }
                    TSPNode childNode = new TSPNode(parentNode.level+1,parentNode.path);
                    childNode.path.add(i);
                    if(childNode.level == numOfVertex-2){
                        childNode.path.add(remainingVertex(childNode.path));
                        childNode.path.add(1);
                        childNode.bound = bound(childNode,graph);
                        if(length(childNode.path,graph) < minLength){
                            minLength = length(childNode.path,graph);
                            optimalTour = childNode.path;
                        }
                    }else{
                        childNode.bound = bound(childNode,graph);
                        if(childNode.bound < minLength){
